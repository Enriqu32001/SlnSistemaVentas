    ...
    fetch("/Venta/ListaCliente")
        ...
        .then(responseJson => {
            if (responseJson.length > 0) {
                responseJson.forEach((item) => {
                    $("#cboCliente").append(
                        $("<option>").val(item.idCliente).text(item.pNySN + ' ' + item.pAySA)
                    )
                })
            }
        })
    ...
    swal({
        ...
        function (valor) {
            ...
            let producto = {
                ...
                impuestoTotal: (parseFloat(valor) * data.precio * porcentaje).toString(),
                total: ((parseFloat(valor) * parseFloat(data.precio) * porcentaje) + parseFloat(parseFloat(valor) * data.precio)).toString()
            }
    ...
    ProductosParaVenta.forEach((item) => {
        ...
        $("#tbProducto tbody").append(
            $("<tr>").append(
                ...
                $("<td>").text(item.precio),
                $("<td>").text(item.precio * item.cantidad),
                $("<td>").text((item.precio * porcentaje * item.cantidad).toFixed(2)),
                $("<td>").text(parseFloat(item.total))
            )
        )
